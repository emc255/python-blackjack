...
class Game:
    def __init__(self, window, player: Player, number_of_deck: int = 1, ):
        self.dealer = Dealer()
        self.player = player
    ...
    def deal_card(self):
        ...
        center_x = (self.window.get_width() - 80) // 2
        center_y = (self.window.get_height() - 100) // 2
        image_x = center_x - 10

        back_image = pygame.transform.scale(pygame.image.load("resource/images/cards/back.png").convert(), (80, 100))
        self.window.blit(back_image, (center_x, center_y))
        ...
        for index in range(number_of_cards_to_deal):
            ...
            if is_player_receive_card:
                ...
                while image_y < 450:
                    ...
                    pygame.draw.rect(self.window, (66, 123, 184), (image_x, image_y, 80, 100), 1)
                    pygame.display.flip()
            else:
                self.dealer.add_card(self.deck.remove_card())
                image_y -= 100
